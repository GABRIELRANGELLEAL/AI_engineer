...
TOOLS: dict[str, dict[str, Any]] = {
    ...
    "execute_code": {
        ...
        "max_retries": 3,
        "failure_check": _is_code_failure,
    },
    "handle_code_error": {
        "description": (
...
def to_neutral_tools(tool_names: list[str]) -> list[dict]:
    """Convert tool names to the provider-neutral format consumed by one_call."""
    return [
        {
            "name": name,
            "description": TOOLS[name]["description"],
...
def is_tool_failure(tool_name: str, output: str) -> bool:
    """Return True if this tool's output indicates a failure worth retrying."""
    check = TOOLS.get(tool_name, {}).get("failure_check")
    return check(output) if check else False


def get_max_retries(tool_name: str) -> int | None:
    """Return the max retry count for a tool, or None if not retryable."""
    return TOOLS.get(tool_name, {}).get("max_retries")


def is_loop_terminator(tool_name: str) -> bool:
    """Return True if this tool's execution should end the agentic loop."""
    return TOOLS.get(tool_name, {}).get("terminates_loop", False)
